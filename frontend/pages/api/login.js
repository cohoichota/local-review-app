...
export default async (req, res) => {
    ...
    if (req.method === 'POST') {
        ...
        try {
            const {data:accessResponse} = await axios.post('http://127.0.0.1:8000/api/token/', body, config)
        ...
        if (accessToken) {
            ...
            const {data:userData} = await axios.get('http://127.0.0.1:8000/api/user/', userConfig)
